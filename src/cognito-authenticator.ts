...
interface RedirectResponseParams {
  /** tokens: Cognito User Pool tokens.*/
  tokens: any;
...
export class CognitoAuthenticator {
   ...
   * @param  {RedirectResponseParams} Parameters for how to handle the redirect
  ...
  async _getRedirectResponse({
    ...
    const directives = !this._disableCookieDomain
      ? `Domain=${domain}; Expires=${new Date(
